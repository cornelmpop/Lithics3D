test_that("getTArea (mesh_area helper) produces correct output", {
  ...
  expect_equal(Lithics3D:::getTArea(validTriangle1), 18)
  expect_equal(round(Lithics3D:::getTArea(validTriangle2), 5), 26.55184)
  ...
  # Bad input = NA
  expect_true(is.na(Lithics3D:::getTArea(invalidTriangle)))
  # Bad input = NA
  expect_true(is.na(Lithics3D:::getTArea(invalidTriangle2)))
  expect_equal(Lithics3D:::getTArea(degenTriangle), 0)
})
