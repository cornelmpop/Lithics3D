test_that("edge_angles_yw works as expected", {
  ...
  expect_identical(round(res$theta, 9), c(97.958889514, 107.921335934))
  expect_identical(round(res$fit, 9), 0.043602052)
  
  res <- edge_angles_yw(mesh, poi, radius = 5, lambda = 0)
  expect_identical(round(res$theta, 9), c(88.417980858, 96.2636612))
  expect_identical(round(res$fit, 9), 0.062016589)
  
  res <- edge_angles_yw(mesh, poi, radius = 5, lambda = 5)
  expect_identical(round(res$theta, 9), c(87.708882574, 97.823776661))
  expect_identical(round(res$fit, 9), 0.05722007)
